...
    // Admin and moderator can see all tickets (default query is {})
...
    // Check access: Admin and Moderator can access, plus project moderator and resolved user.
    const project = ticket.task.project;
    if (
      req.user.role !== 'admin' &&
      req.user.role !== 'moderator' && // FIX: Allow any user with the 'moderator' role to view the ticket
...
    // Check access: Admin can verify, or the assigned Project Moderator can verify.
    const project = ticket.task.project;
    
    const isProjectModerator = project.moderator.toString() === req.user._id.toString();

    if (
      req.user.role !== 'admin' &&
      !isProjectModerator // FIX: Corrected logic to ensure Admin OR Project Moderator access.
    ) {
      return res.status(403).json({ message: 'Access denied' });
    }
...
    // Check if user is admin or the assigned project moderator.
    const project = ticket.task.project;
    if (
      req.user.role !== 'admin' && // Added admin check
      project.moderator.toString() !== req.user._id.toString()
    ) {
      return res.status(403).json({ message: 'Access denied' });
    }
